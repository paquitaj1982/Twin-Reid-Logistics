...
const App: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-black overflow-hidden font-sans text-zinc-200 selection:bg-twin-red selection:text-white">
      ...
      {/* Sidebar */}
      <aside 
        className={`${isSidebarOpen ? 'w-64' : 'w-20'} hidden md:flex flex-col bg-zinc-950 border-r border-zinc-900 transition-all duration-300 ease-in-out relative z-10`}
      >
        <div className="p-6 flex items-center gap-3">
          ...
          <div className={`overflow-hidden transition-all duration-300 ${!isSidebarOpen && 'opacity-0 w-0'}`}>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          <NavItem view="dashboard" icon={LayoutDashboard} label={isSidebarOpen ? "Dashboard" : ""} />
          <NavItem view="dispatch" icon={MapIcon} label={isSidebarOpen ? "Dispatch" : ""} />
          <NavItem view="fleet" icon={Truck} label={isSidebarOpen ? "Fleet Manager" : ""} />
          <NavItem view="academy" icon={GraduationCap} label={isSidebarOpen ? "Academy" : ""} />
          
          <div className="my-6 border-t border-zinc-900 mx-2"></div>
          
          <NavItem view="paperwork" icon={FileText} label={isSidebarOpen ? "Paperwork" : ""} />
        </nav>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        {/* Top Bar */}
        <header className="h-16 border-b border-zinc-900 bg-black/80 backdrop-blur-md flex items-center justify-between px-6 z-20">
